...
$(document).ready(function () {
  ...
  $("#tablaUnidadAdministrativa tbody").on(
    ...
    "button.editUA ",
    function () {
      let dataEditUA = tablaUnidadAdministrativa
        .row($(this).parents("tr"))
        .data();
      ...
      $("#editUAFacultad").append(
        "<option value=" +
          dataEditUA.nombre_facultad.replace(/ /g, "") +
          " selected disabled>" +
          dataEditUA.nombre_facultad +
          "</option>"
      );
  ...
  $("#formBajaUnidadAcademica").submit(function (e) {
    ...
    $.ajax({
      ...
      url: "../controlador/unidadAdministrativa.php",
      data: {
        metodo: "cambioEstadoUA",
        idUA: $("#bajaUA").val(),
        cambioUA: $("#estadoUACambio").val(),
      },
      success: function (response) {
        if (!isNaN(response)) {
          //$('#myModal3').modal('hide');

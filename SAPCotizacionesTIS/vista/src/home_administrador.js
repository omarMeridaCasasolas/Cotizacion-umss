...
$(document).ready(function () {
  ...
  //     e.preventDefault();
  // });
  $("#tablaUnidadAdministrativa tbody").on(
    "click",
    "button.bajaUA",
    function () {
      let dataBajaUA = tablaUnidadAdministrativa
        .row($(this).parents("tr"))
        .data();
  ...
  tablaUnidadAdministrativa = $("#tablaUnidadAdministrativa").DataTable({
    ...
    columns: [
      ...
      },
      {
        data: null,
        defaultContent:
          "<button type='button' class='venderProducto btn btn-warning btn-sm' data-toggle='modal' data-target='#myModal2'><i class='fas fa-edit'></i></button>	<button type='button' class='bajaUA btn btn-danger btn-sm' data-toggle='modal' data-target='#myModal3'><i class='fas fa-trash-alt'></i></button>",
        width: "15%",
      },
    ],
  });
...
function mostrarUnidadDeGastos() {
  ...
  tablaUnidadDeGastos = $("#tablaUnidadDeGastos").DataTable({
    ...
    columns: [
      { data: "nombre_ug" },
      {
        data: "activo_ug", // can be null or undefined
        // "defaultContent": "Sin Asignacion", "width": "15%"},
        render: function (data) {
          if (data == true) {
            return '<h5><span class="badge badge-success">Activo</span></h5>';
